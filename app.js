// ─────────────────────────────────────────────────────────────
//  CALORIE LENS  —  app.js
//  Frontend calls the Python backend at /analyze
//  No API key needed here — key lives in server.py
// ─────────────────────────────────────────────────────────────

// Backend URL — change this when you deploy your Python server
// Local dev:   http://localhost:5000/analyze
// Deployed:    https://your-server.com/analyze
...
async function analyze() {
  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 401) {
        showInvalid("Invalid OpenAI API key on the server. Check OPENAI_API_KEY in server.py");
      } else {
        showInvalid(data?.error || `Server error (${res.status}). Make sure server.py is running.`);
      }
      return;
    }

    if (!data) { showInvalid("Empty response from server. Please try again."); return; }
    if (!data.valid) { showInvalid(data.reason || "Not a recognizable food image. Try a clearer photo."); return; }
    ...
    if (err instanceof TypeError) {
      showInvalid("Cannot connect to backend. Run: python server.py");
    } else {
